...
// if number more than our data base what to do ?????

const numberInput = $.querySelector('#input-number');
const generateBtn = $.querySelector('.generate-btn');
const loremText = $.querySelector(".lorem-text");
...
function generateLoremText(){
  let paragraphCount = numberInput.value;
  resultText = '';

  if(paragraphCount > dummyText.length){
    paragraphCount = dummyText.length;
  }
 
  

  for(let i = 0 ; i < paragraphCount ; i++){
    resultText += dummyText[i] + '<br><br>';
  }

  loremText.innerHTML = resultText;
}